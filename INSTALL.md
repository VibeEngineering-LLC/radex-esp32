...
**Это рекомендуемый путь для платы N16R8** — ничего собирать не нужно, весь остаток документа (после этого раздела и «Требований к оборудованию») тебе не понадобится. Он описывает Путь 2 — сборку прошивки из исходников (Python, ESPHome, `secrets.yaml`, компиляция) — нужную только для классической платы или своих паролей.
