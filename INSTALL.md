...
Прошивальщик — отдельный проект: <https://github.com/VibeEngineering-LLC/atomspectra-waterfall-esp32-flasher>. Он сам скачивает ассет `firmware.factory.bin` из релиза `radex-esp32` и пишет его на плату. **Для этого пути нужен релиз v0.4.2** (ESPHome-ветка): в релизах ESP-IDF-ветки (v1.x, в том числе v1.13.0) ассет с тем же именем `firmware.factory.bin` — полный образ ESP-IDF, другая прошивка (см. «Путь 3»); убедитесь, что выбрана нужная версия.
...
1. **Скачайте образ** из [релиза v1.13.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.13.0):

   | Файл | Когда брать | Адрес записи |
   |---|---|---|
   | `firmware.factory.bin` | плата чистая либо на ней стояла ESPHome-прошивка | `0x0` |
   | `radex-gw-idf.bin` | на плате уже стоит эта же ветка, обновляется только приложение | `0x20000` |
...
3. **Залейте образ.** Полная установка (`COM7` — замените на свой порт; в Linux и macOS это `/dev/ttyUSB0`, `/dev/tty.usbserial-*` и подобные):
   ...
   ```bash
   esptool.py --chip esp32s3 --port COM7 --baud 921600 write_flash 0x0 firmware.factory.bin
   ```

   Обновление приложения на плате, где эта ветка уже стоит:

   ```bash
   esptool.py --chip esp32s3 --port COM7 --baud 921600 write_flash 0x20000 radex-gw-idf.bin
   ```
...
6. **Включите Radex MR107ion рядом с платой.** Адрес прибора вводить не нужно — плата находит его в эфире. Время платы приходит по NTP; сервер и часовой пояс задаются на вкладке «Интеграции», в разделе «Время и дата» (по умолчанию `pool.ntp.org` и `MSK-3`), там же — ручной ввод времени на случай, когда NTP недоступен. До первой синхронизации записи истории копятся с относительными метками и после получения времени переводятся в календарное, а не теряются.
   ...
   > **Прибор можно подключить кабелем** (с v1.13.0): разъём **USB (OTG)** платы — к MR107ion, кабелем с линиями данных. Для питания прибора по USB на плате нужно замкнуть (запаять) перемычку USB-OTG — по опыту проекта atomspectra-waterfall-esp32 (подробности и фото в его [`INSTALL.md`](https://github.com/VibeEngineering-LLC/atomspectra-waterfall-esp32/blob/main/INSTALL.md)); на распространённых клонах ESP32-S3-DevKitC-1 (N16R8) перемычка есть, на оригинальной плате Espressif её нет. Плату питать от блока 5 В / не менее 1 А. Плата сама переходит на USB, как только прибор определится, и возвращается на BLE, когда кабель вынут. Подробности, фото перемычки и ограничения — в [`README.md`](README.md), раздел «Подключение прибора по USB».
...
- **После записи образа плата уходит в цикл перезагрузок.** Чаще всего на плате осталась разметка разделов от ESPHome-прошивки. Сотрите флеш целиком и запишите полный образ заново: `esptool.py --port COM7 erase_flash`, затем запись `firmware.factory.bin` (из релиза ESP-IDF-ветки) на `0x0`.
