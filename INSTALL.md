...
2. [Быстрый путь: готовый образ через прошивальщик](#быстрый-путь-готовый-образ-через-прошивальщик)
3. [Требования к оборудованию](#требования-к-оборудованию)
4. [Требования к системе и софту](#требования-к-системе-и-софту)
5. [Шаг 1: Установить драйвер USB-Serial](#шаг-1-установить-драйвер-usb-serial)
6. [Шаг 2: Установить Python и ESPHome](#шаг-2-установить-python-и-esphome)
7. [Шаг 3: Получить файлы прошивки](#шаг-3-получить-файлы-прошивки)
8. [Шаг 4: Заполнить `secrets.yaml`](#шаг-4-заполнить-secretsyaml)
9. [Шаг 5: Подставить MAC Radex (опционально)](#шаг-5-подставить-mac-radex-опционально)
10. [Шаг 6: Прошить плату — выбери путь А или Б](#шаг-6-прошить-плату--выбери-путь-а-или-б)
11. [Путь А: через Claude Code](#путь-а-через-claude-code)
12. [Путь Б: через стандартный ESPHome (CLI, Web Installer, Dashboard)](#путь-б-через-стандартный-esphome)
13. [Шаг 7: Подключить плату к WiFi (captive portal)](#шаг-7-подключить-плату-к-wifi)
14. [Шаг 8: Открыть Web UI и привязать MAC прибора](#шаг-8-открыть-web-ui)
15. [Шаг 9 (опционально): Подключить к Home Assistant](#шаг-9-подключить-к-home-assistant)
16. [Шаг 10 (опционально): Народмон-выгрузка — что есть и как НЕ включить случайно](#шаг-10-народмон-выгрузка)
17. [Troubleshooting — топ-10 проблем](#troubleshooting)
...
## Быстрый путь: готовый образ через прошивальщик

Всё, что описано ниже начиная с «Требований к системе и софту» — это сборка прошивки из исходников: Python, ESPHome, `secrets.yaml`, компиляция. Если возиться с этим не хочется, есть короткий путь: взять уже собранный образ `firmware.factory.bin` из релизов репозитория и залить его прошивальщиком.
...
Образ собран из `radex_gateway_s3_n16r8.yaml` для платы **ESP32-S3-DevKitC-1 N16R8**. MAC прибора в него не зашит — плата ищет Radex в эфире сама, поэтому один и тот же бинарник годится всем.

Что происходит после заливки:

1. Плата ищет сеть `radex-gw-setup`. Такой сети не существует, это сделано намеренно — не найдя её, плата поднимает свою точку доступа **`radex-gw-s3 Fallback`**, пароль `radexgw123`.
...
3. Открой `http://radex-gw-s3.local/` (если mDNS не работает — по IP, который выдал роутер). Логин `radex`, пароль `radex`.
4. MAC прибора вводить не нужно. Плата находит Radex по имени в эфире (`MR107*` / `Radex*`) или по UUID сервиса `FE651700-…`, запоминает адрес и тип адреса в NVS и после перезагрузки берёт их из памяти. Найденный адрес виден в группе «Сервис» в поле «Найденный прибор Radex»; сбросить привязку — кнопкой «Забыть прибор и искать заново».
5. Если прибор нужно указать вручную (например, рядом несколько Radex), поле «MAC Radex» в Web UI осталось на месте и имеет приоритет над автопоиском.

На проверке прошивка нашла прибор Radex MR107ion в течение минуты после старта и стала отдавать показания. Адрес прибора оказался random-типа, поэтому вместе с самим адресом сохраняется и его тип — без этого подключение к такому прибору молча не состоялось бы.
...
Дальше по документу — путь сборки из исходников. Он же нужен, если ты хочешь свои пароли, шифрование ESPHome API или другую плату.
...
https://github.com/VibeEngineering-LLC/radex-esp32 (папка firmware/).
