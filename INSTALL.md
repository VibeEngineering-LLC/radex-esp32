...
6. **Включите Radex MR107ion рядом с платой.** Адрес прибора вводить не нужно — плата находит его в эфире. Время платы приходит по NTP; сервер и часовой пояс задаются на вкладке «Интеграции», в разделе «Время и дата» (по умолчанию `pool.ntp.org` и `MSK-3`), там же — ручной ввод времени на случай, когда NTP недоступен. До первой синхронизации записи истории копятся с относительными метками и после получения времени переводятся в календарное, а не теряются.
   ...
   > **Прибор можно подключить кабелем** (с v1.13.0): разъём **USB (OTG)** платы — к MR107ion, кабелем с линиями данных. Перед этим на плате нужно **замкнуть перемычку USB-OTG**, иначе разъём не даёт 5 В; плату питать от блока 5 В / не менее 1 А. Плата сама переходит на USB, как только прибор определится, и возвращается на BLE, когда кабель вынут. Подробности, фото перемычки и ограничения — в [`README.md`](README.md), раздел «Подключение прибора по USB».
