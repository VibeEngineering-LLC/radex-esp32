...
### INC-FW18-001 — прибор с прошивкой **1.8** не отдаёт GATT ни одному центральному на ESP32

**Симптом.** Плата подключается к прибору, но данные не идут: в Web UI `успешных коннектов = 0`, в логе цикл «подключились → разрыв через 5–9 с» с кодом `reason = 8`. Прибор при этом штатно работает с телефоном и с ПК.

**Причина (установлена измерением, не устранена).** Прибор MR107ion с прошивкой **v1.8** отвечает только на `Exchange MTU Request`, который инициирует сам, и не отвечает ни на один последующий ATT-запрос, если центральное устройство — ESP32/ESP32-S3. Воспроизводится на двух платах, на Bluedroid и на NimBLE одинаково; тот же клиент против эталонного `gatt_server` из ESP-IDF работает нормально. Прибор с прошивкой **1.7** с тем же шлюзом работает штатно (19 ч аптайма, 0 реконнектов).

**Что делать сейчас.** Обходного пути на стороне шлюза не найдено. Если прибор ещё на 1.7 — **не обновлять**. Вопрос передан изготовителю.

**Подробности и все замеры:** [`references/mr107ion-fw18-ble-report.md`](references/mr107ion-fw18-ble-report.md) · обсуждение — [issue #1](https://github.com/VibeEngineering-LLC/radex-esp32/issues/1).

**Сопутствующая находка (исправлено у нас).** Прибор не обслуживает ничего, пока не получит ответ на свой `Exchange MTU Request`, — центральному нужен включённый GATT-сервер. В ESPHome это компонент `esp32_ble_server:` либо `sdkconfig_options: CONFIG_BT_GATTS_ENABLE: y`.
