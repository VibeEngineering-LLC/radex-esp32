...
[релизе v1.2.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.2.0):
`radex-gw-idf-v1.2.0-factory.bin` (полный образ, заливать на `0x0`) и
...
**Обязательно: перемычка USB-OTG.** Без неё разъём OTG не выдаёт 5 В, и прибор по USB
не питается и не определяется. На обратной стороне платы надо замкнуть (запаять)
перемычку **USB-OTG**:

![Перемычки на плате — замкните USB-OTG](images/solder-jumper.webp)

*Фото и формулировка — из соседнего проекта того же владельца,
...
- **Питание.** Прибор заряжается от платы. Один раз при подключении прибора наблюдалась
  перезагрузка платы по питанию (`POWERON`) — вероятно, просадка напряжения от тока заряда.
...
- **Порт эксклюзивен.** Пока прибор подключён к плате, RadexDC с ним работать не может, и
  наоборот.
- **Запись в прибор — только часы.** Из всех команд записи шлюз отправляет одну — установку
  часов прибора (RPC `0x0006`), и только после синхронизации платы по NTP, при расхождении
  больше 5 с и не чаще раза в 6 ч. Все прочие запросы проходят белый список из шести команд
  (`main/radex_ekosf.c`, `radex_req_allowed()`); сброс, выключение, загрузчик, удаление
  файлов и загрузка прошивки не отправляются ни при каких условиях.
