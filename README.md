...
| Релизы | v0.1.0 – v0.5.0 | [v1.0.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.0.0) – [v1.13.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.13.0) |
...
> v1.13.0, и это ветка ESP-IDF: ассет `firmware.factory.bin` в нём тоже есть (как и в v1.11.1
...
## Альтернативная архитектура: чистый ESP-IDF (v1.2.0)
...
[релизе v1.13.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.13.0):
...
- **Запросы к прибору — белый список из шести** (`main/radex_ekosf.c`, `radex_ekosf.h`,
  ...
  полностью, кадры повторяют трассы RadexDC) и одна **запись** — установка часов (`0x0006`, только
  после синхронизации платы по NTP, при расхождении больше 5 с и не чаще раза в 6 ч). Все прочие RPC
  отклоняются до отправки; сброс, выключение, загрузчик, удаление файлов и загрузка прошивки не
  отправляются ни при каких условиях.
- **ВНИМАНИЕ: установка часов может начать новую сессию замера на приборе.** По наблюдению за
  RadexDC установка времени закрывает текущую сессию прибора и открывает новую. Шлюз шлёт ту же
  команду `0x0006`, и синхронизация часов **включена по умолчанию**, поэтому он может сам закрыть
  идущую сессию, и «средняя сессии» (`radon_avg`) начнётся заново. **На плате `0x0006` не
  проверялась**: поведение шлюза выведено из наблюдения за RadexDC. Отключается сборкой: параметр
  `CONFIG_RADEX_USB_CLOCK_SYNC=n` в `firmware-idf/sdkconfig.defaults` (menuconfig → «Radex MR107ion
  gateway» → «USB: выставлять часы прибора (RPC 0x0006) по NTP»); в готовом образе переключателя нет.
