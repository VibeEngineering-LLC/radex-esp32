...
| Релизы | v0.1.0 – v0.5.0 | [v1.0.0](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.0.0) – [v1.13.1](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.13.1) |
| Канал связи с прибором | BLE | BLE и USB-кабель (с v1.13.0), выбор автоматически — раздел «[Подключение прибора по USB](#подключение-прибора-по-usb-esp-idf-с-v1130)» |
...
«[Альтернативная архитектура: чистый ESP-IDF](#альтернативная-архитектура-чистый-esp-idf)».
...
> v1.13.1, и это ветка ESP-IDF: ассет `firmware.factory.bin` в нём тоже есть (как и в v1.11.1
> и v1.12.0), но это **полный образ ESP-IDF-ветки**, а не ESPHome; рядом лежит
> `radex-gw-idf.bin` (только приложение). Имя ассета одно и то же, поэтому берите именно
> v0.4.2. Образ ESPHome-ветки берётся из v0.4.2 (последний стабильный) либо
...
1. **Залейте образ.** Прошивальщик сам скачает `firmware.factory.bin` из релиза (v0.4.2 — см. выше). Плате
...
«[Альтернативная архитектура](#альтернативная-архитектура-чистый-esp-idf)».
...
Для `radex_gateway_s3_n16r8.yaml` в релизах v0.4.2 и v0.5.0 лежит готовый образ
`firmware.factory.bin` (в релизах ESP-IDF-ветки v1.x ассет с тем же именем — другая прошивка,
см. выше). Его умеет заливать
...
## Альтернативная архитектура: чистый ESP-IDF
...
- Шесть вкладок вместо одной плоской таблицы («Результаты измерений», «Соответствие
  нормативу», «Метод», «Интеграции», «Графики», «Системные»), полноэкранный график,
  адаптация раскладки под мобильный экран.
- **Доверительный интервал расчётной среднегодовой ОА радона на графике** — полоса
  `c(t)·(1 ± √(UV(t)² + U_D²))`, сужающаяся по мере набора длительности: момент, когда
  её верхняя граница опускается ниже норматива, и есть «соответствует». Различаются
  две величины с общим центром: измеренная **средняя ОА радона** за тест и
  **расчётная среднегодовая**, у которой есть неопределённость по формуле критерия.
- **Сохранение замера под своим именем** — кнопка «Сохранить замер» снимает идущий
  замер в отдельный файл на флеш и подписывает его; список сохранённых замеров с
  именами — на вкладке «Графики», там же заключение по любому из них
  (`/api/assess?start=<epoch>`) отдельно от текущего замера.
...
[релизе v1.13.1](https://github.com/VibeEngineering-LLC/radex-esp32/releases/tag/v1.13.1):
`firmware.factory.bin` (полный образ, заливать на `0x0`) и
`radex-gw-idf.bin` (только приложение, заливать на `0x20000`). Контроль целостности —
`sha256sums.txt` в том же релизе. Пошаговый порядок заливки — в
[`INSTALL.md`](INSTALL.md), раздел «Путь 3».

**Плату, которая уже настроена, обновлять только приложением** (`0x20000`). Полный
образ — непрерывный от `0x0`, и в его диапазон попадает раздел `nvs`: заливка на
`0x0` стирает сохранённую сеть, привязку прибора и параметры критерия, после чего
плата поднимает точку `RadexGW-Setup` и настройку приходится проходить заново.
Раздел с историей измерений (`storage`, `0x620000`) лежит за концом образа и не
затрагивается ни в том, ни в другом случае.
...
### Подключение прибора по USB (ESP-IDF, с v1.13.0)

С v1.13.0 ESP-IDF-ветка умеет читать MR107ion не только по BLE, но и по кабелю USB:
плата работает USB-хостом, прибор подключается к ней как устройство CDC-ACM
(VID:PID `ABBA:A204`, 230400 бод). Кабель снимает все беды радиоканала — разрывы,
очередь команд, дележ эфира с Wi-Fi, — ценой того, что прибор перестаёт быть
автономным и привязан проводом к плате. Для стационарного мониторинга это
приемлемо, для носимого прибора — нет, поэтому BLE остаётся в той же прошивке.

**Схема.** На ESP32-S3-DevKitC-1 N16R8 **два** разъёма USB-C:

```
  ПК или блок питания 5 В ──USB──► [ UART ]  ESP32-S3-DevKitC-1 N16R8  [ USB (OTG) ] ◄──USB── Radex MR107ion
                                   питание, прошивка, лог                прибор (кабель С ЛИНИЯМИ ДАННЫХ)
```

- Разъём **UART** — питание платы, прошивка и лог, как и раньше.
- Разъём **USB (OTG)** — прибор. Кабель нужен **с линиями данных**: «зарядный» кабель
  даёт в Windows «неопознанное USB-устройство», на плате — просто ничего.

**Перемычка USB-OTG.** Для питания прибора по USB на плате нужно замкнуть (запаять) перемычку
USB-OTG — по опыту проекта atomspectra-waterfall-esp32, подробности и фото там
([`INSTALL.md`](https://github.com/VibeEngineering-LLC/atomspectra-waterfall-esp32/blob/main/INSTALL.md)).
Перемычка есть на распространённых клонах ESP32-S3-DevKitC-1 (N16R8); на оригинальной плате
Espressif такой перемычки нет, питание OTG-разъёма зависит от исполнения платы.

![Перемычки на плате — замкните USB-OTG](images/solder-jumper.webp)

*Фото — из соседнего проекта того же владельца,
[atomspectra-waterfall-esp32](https://github.com/VibeEngineering-LLC/atomspectra-waterfall-esp32)
(`INSTALL.md`, `images/solder-jumper.webp`, лицензия MIT).*

**Как выбирается канал.** Ничего переключать не нужно — плата решает сама, на лету и без
перезагрузки:

| Ситуация | Канал | Что происходит с BLE |
|---|---|---|
| Прибор определился на USB | USB | На паузе: стек поднят, но не ищет и не подключается |
| USB нет (или выдернули кабель — через 10 с) | BLE, как в v1.12.0 | Поиск, выбор прибора по MAC, опрос |
| Bluetooth выключен в настройках и USB нет | нет | Не запускается |

Задержка 10 с при пропаже USB нужна, чтобы кратковременное переоткрытие порта не дёргало
BLE туда-обратно. Активный канал виден на странице — приставкой «USB · » / «BLE · » к
строке статуса опроса — и в `GET /api/data` полем `transport` (`usb` | `ble` | `none`).

**Выключатель Bluetooth** — флажок «Bluetooth (когда прибор не на USB)» в карточке
«Прибор (MAC)» на вкладке «Системные» (API: `POST /api/bt`, тело `1` / `0`; состояние —
поле `bt_enabled` в `/api/system`). Хранится в памяти платы и переживает перезагрузку, по
умолчанию включён. Выключенный Bluetooth означает «только USB»: BLE-клиент не запускается
вовсе.

**Что читается по USB.** Текущие показания (раз в 10 минут, как по BLE): ОА последнего
цикла, средняя сессии прибора и её СКО, температура, влажность. Журнал прибора — кнопкой
«Прочитать журнал с прибора», последние 64 записи, в том числе через границу сессий прибора
(новая сессия начинается, например, после установки часов в RadexDC); сохранение в историю —
как для BLE-журнала. Колонка «ОА» журнала — скользящее среднее прибора (как в BLE-журнале; в
v1.13.0 по USB по ошибке отдавалась сырая ОА, исправлено в v1.13.1). По BLE журнал читает только
сессию 0 прибора (см. [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md)).

**Ограничения USB:**

- **Питание.** Прибор заряжается от платы. Один раз при подключении прибора наблюдалась
  перезагрузка платы по питанию (`POWERON`) — возможна просадка напряжения от тока заряда
  (не подтверждено).
  Питайте плату от нормального блока **5 В / не менее 1 А**, а не от USB-порта ноутбука.
- **Кабель** — только с линиями данных (см. выше).
- **Порт эксклюзивен.** Пока прибор подключён к плате, RadexDC с ним работать не может, и
  наоборот.
- **Запросы к прибору — белый список из шести** (`main/radex_ekosf.c`, `radex_ekosf.h`,
  `radex_req_allowed()`): три чтения (`0x0BC2` показания, `0x0C0D` заголовок архива, `0x0C0E`
  страницы архива), два позиционирования архива (`0x0C04`, `0x0C05`; смысл параметров вскрыт не
  полностью, кадры повторяют трассы RadexDC) и одна **запись** — установка часов (`0x0006`; с
  v1.13.1 по умолчанию не вызывается, см. ниже). Все прочие RPC отклоняются до отправки; сброс,
  выключение, загрузчик, удаление файлов и загрузка прошивки не отправляются ни при каких условиях.
- **Установка часов прибора выключена по умолчанию (с v1.13.1).** По наблюдению за RadexDC установка
  времени закрывает текущую сессию замера на приборе и открывает новую, а с платы `0x0006` не
  проверялась — поэтому шлюз часы прибора не выставляет; в образе кода синхронизации нет.
  **ВНИМАНИЕ, если включить** (`CONFIG_RADEX_USB_CLOCK_SYNC=y` в `firmware-idf/sdkconfig.defaults` и
  пересборка; menuconfig → «Radex MR107ion gateway» → «USB: выставлять часы прибора (RPC 0x0006) по
  NTP»): шлюз после синхронизации платы по NTP при расхождении часов больше порога
  (`CONFIG_RADEX_USB_CLOCK_MAX_DRIFT_S`, по умолчанию 5 с; не чаще раза в 6 ч) сам пошлёт `0x0006`,
  а это может начать на приборе новую сессию — «средняя сессии» (`radon_avg`) начнётся заново.
  Поведение шлюза выведено из наблюдения за RadexDC, на плате не проверялось. Переключателя в
  работе (без пересборки) нет. В v1.13.0 синхронизация была включена по умолчанию.
...
│   └── archive/                       ← старые YAML
├── firmware-idf/                      ← ВЕТКА ESP-IDF (релизы v1.x), только ESP32-S3 N16R8
