...
// #USB-1 универсальная сборка: только для radex_link.c (не входят в шов, не переименовываются).
void ble_radex_pause(bool on);      // пауза BLE: стек поднят, но без скана/соединения/попыток открыть
bool ble_radex_stack_up(void);      // контроллер и Bluedroid подняты
