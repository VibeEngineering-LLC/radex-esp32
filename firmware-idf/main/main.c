...
/* #RADEX-263: публикации живут в СВОЕЙ задаче. Разбор — у вызова ниже. */
...
void app_main(void)
{
    ...
    /* #RADEX-263: задача публикаций создаётся ДО BLE. Причина: ble_radex_start()
       управление НЕ возвращает — её собственный while(1) опрашивает прибор
       каждые 100 мс (конец ble_radex.c). Стоявший ниже хвостовой цикл app_main
       был поэтому недостижим, и ha_mqtt_publish()/narodmon_publish() не
       вызывались ни разу за всё время работы платы. Стек 6144 — getaddrinfo и
       сокет narodmon-отправки не помещаются в 4096 с запасом; приоритет 3 —
       ниже задачи переподключения BLE (5), выше main (1). */
    xTaskCreate(publish_task, "publish", 6144, NULL, 3, NULL);

    // #RADEX-186: в режиме первичной настройки BLE не поднимаем. WiFi и BLE делят
    // одно радио, а непрерывный поиск прибора в эфире отнимает время ровно тогда,
    // когда пользователь подключается к точке доступа и грузит страницу настройки.
    // Прибор в этот момент и не нужен: после сохранения сети плата перезагрузится.
    if (wifi_manager_mode() == NET_MODE_SETUP) {
        ESP_LOGW(TAG, "режим настройки: BLE не запускаю, эфир отдан точке доступа");
    } else {
        ESP_LOGI(TAG, "запускаю BLE-клиент к прибору");
        ble_radex_start(on_measurement);
    }

    /* Сюда исполнение доходит только в режиме первичной настройки: BLE тогда не
       запускали и ble_radex_start() не съела задачу. Публикации идут своей
       задачей и от этого цикла не зависят. */
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(30000));
    }
}
