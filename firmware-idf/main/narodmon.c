...
void narodmon_publish(void) {
    /* #RADEX-263: пульс вызова. Оба штатных выхода (выключено / интервал не
       истёк) писали ESP_LOGD, а уровень журнала на плате — INFO, поэтому в
       журнале они не видны вовсе. Из-за этого «функцию не вызывают» и
       «функция вызвана и вышла штатно» выглядели одинаково: 07.09 на живой
       плате при enabled=true, заданном адресе и валидных показаниях за 9 минут
       не появилось НИ ОДНОЙ строки, и причину нельзя было назвать по журналу.
       Раз в 10 вызовов (≈5 мин при цикле в app_main 30 с) печатаем строку с
       причиной пропуска — теперь тишина означает именно отсутствие вызова. */
    static uint32_t s_calls = 0;
    const bool pulse = ((s_calls++ % 10) == 0);

    // Проверяем, включено ли
    if (!g_enabled) {
        if (pulse) ESP_LOGI(TAG, "пульс: выгрузка выключена, отправки нет");
        return;
    }
    ...
    if (g_last_send_time > 0 && (now - g_last_send_time) < interval_us) {
        if (pulse) ESP_LOGI(TAG, "пульс: до следующей отправки %lld с",
                            (long long)((interval_us - (now - g_last_send_time)) / 1000000));
        return;
    }
    ...
    int len = snprintf(send_buffer, sizeof(send_buffer),
                       /* #RADEX-263: метрика радона — RR1, как в ESPHome-ветке
                          (radex_gateway.yaml, nm_radon). С прежним R1 narodmon.ru
                          по префиксу «R» определял тип датчика как «радиация» и
                          показывал объёмную активность радона в мкР/ч: 07.09 на
                          публичной карте стояло «109 µR/h» вместо 109 Бк/м3, то
                          есть величина была названа чужим именем и читалась как
                          двадцатикратный фон. Тип на сайте правится руками, но
                          рассчитывать на это у каждого владельца платы нельзя. */
                       "#%s#Radex MR107ion\n"
                       "#RR1#%.1f\n"
                       "#T1#%.1f\n"
                       "#H1#%.0f\n"
    ...
    /* #RADEX-263: попытка отправки видна в журнале ДО сетевых операций.
       Раньше первая запись появлялась только по её итогу, и «попытки не было»
       не отличалось от «попытка зависла в getaddrinfo/connect». Печатаем
       адрес станции — он же главный предмет разбирательств с narodmon.ru,
       который на чужой адрес отвечает «Device MAC is invalid». */
    ESP_LOGI(TAG, "отправляю на narodmon.ru:8283, станция %s", g_station_mac);
