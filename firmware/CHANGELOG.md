...
**Добавлено:**
- **Автопоиск прибора.** Лямбда `on_ble_advertise` ищет в эфире устройство с именем,
  начинающимся на `MR107` или `Radex`, либо рекламирующее сервис
  `FE651700-00B0-4240-BA50-05CA45BF8AAA`, и ставит найденный адрес клиенту в рантайме.
  ...
  Тип адреса хранится не для симметрии: у проверенного прибора адрес оказался
  random-типа, а `set_address()` тип не меняет и оставляет PUBLIC — без сохранённого
  типа подключение к такому прибору молча не состоится.
- **Ручной ввод MAC сохранён** и имеет приоритет над автопоиском: поле «MAC Radex» в
  Web UI работает как раньше, оно нужно, когда рядом несколько приборов.
