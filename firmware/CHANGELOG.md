...
Номер версии здесь и тег релиза на GitHub — одно и то же число. Ранний тег `v0.1.0`
(первый публичный релиз, июль 2026) выпал из этой нумерации: он считался от нуля, тогда
как прошивка к тому моменту уже была версии 0.3.0. С v0.4.0 расхождение устранено.


## v0.4.1 (2026-08-24) — пароль точки настройки упрощён

Пароль fallback-AP `radex-gw-s3 Fallback` (`ap_password` в `secrets.public.yaml`) заменён с
`radexgw123` на `radex123` — тот же корень слова, что у логина/пароля Web UI (`radex`/`radex`),
проще набрать на телефоне при первичной настройке. Голое `radex` (5 симв.) технически
невозможно — WPA2 требует минимум 8. Только публичная сборка N16R8, только `ap_password`.

## v0.4.0 (2026-08-20) — публичная сборка под ESP32-S3-DevKitC-1 N16R8: профиль платы задан явно, прибор находится сам

**Что:** новый файл `firmware/radex_gateway_s3_n16r8.yaml` — версия шлюза под плату
**ESP32-S3-DevKitC-1 N16R8** (16 МБ Flash, 8 МБ PSRAM octal). Собирается в образ,
который годится всем, а не только своему автору: MAC прибора в прошивку не зашит.
Именно из этого файла собран ассет `firmware.factory.bin` в релизе — его тянет
прошивальщик (`VibeEngineering-LLC/atomspectra-waterfall-esp32-flasher`).

**Почему:** у базового `radex_gateway_s3.yaml` два ограничения, из-за которых готовым
бинарником им поделиться нельзя. Первое — `ble_client.mac_address` требуется схемой
на этапе компиляции, то есть каждый пользователь обязан пересобрать прошивку под свой
прибор. Второе — профиль памяти платы в YAML не задан, и обе половины маркировки
N16R8 пропадают впустую.

**Добавлено:**
- **Автопоиск прибора.** Лямбда `on_ble_advertise` ищет в эфире устройство, имя которого
  начинается на `MR107` или `Radex` (регистр не важен), либо рекламирующее сервис
  `FE651700-00B0-4240-BA50-05CA45BF8AAA`; принимается и вариант с последним байтом
  `…8AAB` — в reverse-таблице прибора эти два написания соседствуют, и лишняя проверка
  дешевле непойманного прибора. Найденный адрес ставится клиенту в рантайме.
  Адрес и тип адреса сохраняются в NVS (`globals` с `restore_value: true`) и
  переживают перезагрузку — после ребута плата берёт их из памяти, не дожидаясь эфира.
  Уточнение по типу адреса: ESPHome восстанавливает его сам при каждом обнаружении
  прибора в рекламе (`esp32_ble_client/ble_client_base.cpp`, `parse_device()`), поэтому
  хранение типа в NVS — подстраховка, а не обязательное условие подключения.
- **Ручной ввод MAC сохранён** и имеет приоритет над автопоиском: поле
  «MAC Radex (AA:BB:CC:DD:EE:FF)» в Web UI работает как раньше, оно нужно, когда рядом
  несколько приборов.
- **Web UI:** поле «Найденный прибор Radex» (показывает найденный адрес), кнопка
  «Забыть прибор и искать заново» (чистит NVS и запускает поиск с нуля), датчики
  «Свободная память» и «Свободная PSRAM» — второй сразу показывает, включилась ли
  внешняя память платы.
- **`firmware/secrets.public.yaml`** — набор значений, с которым собран публичный
  бинарник. Сеть `radex-gw-setup` заведомо не существует, поэтому плата поднимает
  точку доступа `radex-gw-s3 Fallback` (пароль `radexgw123`), вход в Web UI —
  `radex` / `radex`, пароль OTA — `radexota`. Все они опубликованы и потому секретом
  не являются: кому нужны свои — вписывает их в `secrets.yaml` и пересобирает.
- **`api:` без ключа шифрования.** Сборка публичная, и один общий для всех ключ
  защитой не является. Кому нужно шифрование ESPHome API — добавляет блок
  `encryption` и пересобирает. Web UI при этом закрыт Basic Auth.

**Исправлено — профиль памяти платы:**
- `esp32.flash_size: 16MB`. Без этой строки ESPHome берёт дефолтные 4 МБ
  (`esphome/components/esp32/__init__.py:1575`) и на N16R8 использует четверть флеша.
- Блок `psram: {mode: octal, speed: 80MHz}`. Определение платы в PlatformIO PSRAM не
  включает вовсе, то есть без этого блока 8 МБ внешней памяти платой не используются.
- Итог правки, подтверждённый сборкой: раздел приложения **7.75 МБ** (два OTA-слота по
  `0x7C0000`), `board_build.arduino.memory_type = qio_opi`, флаг `-DBOARD_HAS_PSRAM`.

**Проверено на железе:** плата в течение минуты после старта нашла прибор Radex
MR107ion сама и отдаёт показания — объёмная активность радона, температура,
влажность.

**Не менялось:**
- Народмон-выгрузка **выключена по умолчанию**, switch «Выгружать на Народмон»
  сохраняет `restore_mode: ALWAYS_OFF` — после любой перезагрузки снова OFF.
- READ-poll по GATT-хендлам через `include/radex_read_hook.h`, скользящие средние за
  час и за сутки, Web Server v3 с группами «Данные» / «Сервис» — унаследованы от
  базовой версии без изменений.
- Остальные YAML (`radex_gateway_s3.yaml`, `radex_gateway.yaml`,
  `radex_gateway_v2.yaml`, `radex_gateway_s3_baseline.yaml`) не тронуты.
