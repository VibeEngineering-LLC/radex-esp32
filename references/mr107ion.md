...
### Закрыто прямым ПК-дампом (2026-08-25, #RADEX-4 петля)

- [x] ~~Прочитать 0x2901 (User Description) для всех handles `0x21-0x6f`~~ —
  сделано `scripts/radex4_bleak_user_desc.py` (прямое подключение с ПК через
  `bleak`, в обход ESP32). **Прибор сам называет все 15 полей сервиса
  FE651700, имена ДОСЛОВНО совпали с гипотезой ниже** (составленной ранее по
  анализу паттернов без доступа к дескрипторам): `OAR_sred`, `Sko_OAR_sred`,
  `t_izm_last`, `OAR_last`, `OAR_mov_avr`, `Qakk`, `OAR_min`, `OAR_max`,
  `temper_x10`, `preassure`, `humidity`, `quantity_izm`, `num_izmer`,
  `t_prepare`, `mis_psw`. Дополнительно синхронный снимок дисплея прибора
  (Rn=91 Бк/м³, T=28°C, влажность=43%) сверен с READ по Value-handle
  (`OAR_last`=96.0, `temper_x10`→28°C, `humidity`=43% — точное/близкое
  совпадение) — карта handle подтверждена дважды независимо (имена от
  прибора + значения от READ vs дисплей).

### Config-сервис FE651600 — частично расшифрован прямым READ (2026-08-25)

READ всех 14 characteristics через `scripts/radex4_bleak_gatt_map.py` (сервис не имеет
0x2901-дескрипторов — имена НЕ от прибора, ниже гипотезы по значению/паттерну, не факт):

| Decl handle | UUID | Сырое | Декод | Гипотеза | Уверенность |
|---|---|---|---|---|---|
| 0x0038 | fe651612 | `cd 7d 8d 6a` | u32 LE = 1787657677 | **Unix timestamp RTC** | ✅ подтверждено — декодируется в `2026-08-25 11:34:37 UTC`, точно совпадает с `11:36` на синхронном скриншоте дисплея прибора |
| 0x0024 | fe651602 | `3c` | u8 = 60 | Интервал измерения, сек | гипотеза, не проверено экспериментом |
| 0x002a/0x002c | fe651605/606 | `00 00 48 43` (оба поля идентичны) | f32 = 200.0 | Порог тревоги, Бк/м³ (разумно по НРБ-диапазону) | гипотеза, не проверено |
| 0x0022 | fe651601 | `00` | u8 = 0 | неизвестно | не расшифровано |
| 0x0026 | fe651603 | `05` | u8 = 5 | неизвестно | не расшифровано |
| 0x0028 | fe651604 | `03` | u8 = 3 | неизвестно | не расшифровано |
| 0x002e/0x0030 | fe651607/608 | `00`/`00` | u8=0/0 | неизвестно | не расшифровано |
| 0x0032 | fe651609 | `81 01` | u16 LE = 385 | неизвестно | не расшифровано |
| 0x0034 | fe651610 | `20 22` | u16 LE = 8736 | =24×364, возможно связано с годовым циклом | слабая гипотеза |
| 0x0036 | fe651611 | `00 08` | u16 LE = 2048 | неизвестно (степень 2) | не расшифровано |
| 0x003a/0x003c | fe651613/614 | `00`/`00` | u8=0/0 | неизвестно | не расшифровано |

**Метод для дальнейшей расшифровки (не сделан в этом тике):** контролируемый эксперимент —
изменить настройку через штатное приложение RadexM/кнопки прибора, повторить
`radex4_bleak_gatt_map.py` до/после, сравнить diff по handle. Требует участия оператора
(доступ к прибору + приложению), не выполнимо автономно.
